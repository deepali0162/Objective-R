...
sms_spam <- read.csv('/Users/deepali/Desktop/R/Text Mining/SMS Spam/sms_spam.csv', stringsAsFactors = FALSE)
...
spam <- subset(sms_raw_train, type == "spam") #564
ham  <- subset(sms_raw_train, type == "ham") #3605
#564+3605=4169
...
sms_dict<-findFreqTerms(sms_dtm_train, 3) #removing words for frequency less than 3
#sms_dict <- Dictionary(findFreqTerms(sms_dtm_train, 5))
sms_train <- DocumentTermMatrix(sms_corpus_train, list(dictionary = sms_dict))
sms_test  <- DocumentTermMatrix(sms_corpus_test, list(dictionary = sms_dict))

# convert counts to a factor
#if frequency is 0 then keep it 1 else 0 
...
# margin=1 is row wise and 2 is col wise : line 62/63
